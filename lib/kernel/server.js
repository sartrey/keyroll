 ...
 * @param {Object} input
 */
function serveData(context, route, input) {
  const headers = {};
  headers['Content-Type'] = 'application/json';
  context.response.writeHead(200, headers);
  context.response.async = true;
  try {
    const action = require('../server' + route)
    ...
    result
      ...
      .catch(error => {
        context.response.write(JSON.stringify({ state: false, error: error.message }))
        context.response.end()
      });
  } catch (error) {
    context.response.write(JSON.stringify({ state: false, error: error.message }));
  ...
  if (url.pathname.startsWith(prefixData)) {
    if (request.method === 'POST' || request.method === 'PUT') {
      ...
      request.on('end', () => {
        ...
        serveData(
          context, 
          url.pathname.replace(prefixData, ''), 
          { query: url.query, headers: request.headers, body: buffer }
        );
      });
    } else {
      serveData(
        context,
        url.pathname.replace(prefixData, ''),
        { query: url.query, headers: request.headers }
      );
    }
    return;
  }
